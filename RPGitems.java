...
final class DarkHoldsItems {
    ...
    public static GreatSwordItem greatSword() {
    ...
    public static DragonEggItem dragonEgg() {
    return new DragonEggItem();
    }
    ...
    public static ScrollOfLifeStealItem scrollOfLifeSteal() {
        return new ScrollOfLifeStealItem();
    }

    public static ScrollOfLevelUpItem scrollOfLevelUp() {
        return new ScrollOfLevelUpItem();
    }

    public static DecayedArmorItem decayedArmor() {
        return new DecayedArmorItem();
    }
    ...
    public static DragonScaleShieldItem dragonScale() {
    ...
    public static KeyItem ancientRelic() {
        return new KeyItem("dragon_lair_secret_door", "Ancient Relic");
    }
...
class ScrollOfFireballItem extends GameItem {
    ...
    public ScrollOfFireballItem() {
        super("Scroll of Fireball", ItemType.SPECIAL, FIREBALL_DAMAGE,
                "Deals 25 damage to the current enemy during battle.");
    }
...
class ScrollOfLifeStealItem extends GameItem {
    private static final int LIFE_STEAL_AMOUNT = 15;

    public ScrollOfLifeStealItem() {
        super("Scroll of Life Steal", ItemType.SPECIAL, LIFE_STEAL_AMOUNT,
                "Deals 15 damage to the current enemy and restores 15 HP to the hero during battle.");
    }
    ...
    public boolean use(Player player, ItemContext context) {
        if (context == null || !context.isInBattle() || context.getCurrentEnemy() == null) {
            System.out.println("Scroll of Life Steal can only be used during battle.");
            return false;
        }

        Enemy enemy = context.getCurrentEnemy();
        int enemyHpBefore = enemy.getHp();
        enemy.takeDamage(LIFE_STEAL_AMOUNT);
        int damageDealt = enemyHpBefore - enemy.getHp();

        int playerHpBefore = player.getHp();
        player.heal(LIFE_STEAL_AMOUNT);
        int healed = player.getHp() - playerHpBefore;

        System.out.println("Dark energy drains " + damageDealt + " HP from " + enemy.getName()
                + " and restores " + healed + " HP to the hero.");
        return true;
    }
}

class ScrollOfLevelUpItem extends GameItem {
    private static final int XP_GAIN = 70;

    public ScrollOfLevelUpItem() {
        super("Scroll of Level Up", ItemType.SPECIAL, XP_GAIN,
                "The hero reads the scroll and gains 70 experience points immediately.");
    }
...
class DragonEggItem extends GameItem {
    ...
    public DragonEggItem() {
        super(
            ...
            ItemType.SPECIAL,
            0,
            "Real dragon treasure, can be sold with extreme value."
        );
    }
...
class StickItem extends WeaponItem {
    public StickItem() {
        super("Stick", 2, 95, "Stick Attack",
                "Wooden Stick, hardly can be called a weapon. Weak but reliable.");
    }
...
class SwordItem extends WeaponItem {
    public SwordItem() {
        super("Sword", 8, 90, "Sword Slash",
                "Hilt & iron blade, every knight's best argument.");
    }
}

class GreatSwordItem extends WeaponItem {
    public GreatSwordItem() {
        super("Great Sword", 12, 60, "Great Sword Slash",
                "A very heavy sword yet more effective. Accuracy scales with level.");
    }
    ...
    public int getAccuracyForLevel(int level) {
        if (level < 5) {
            return 60;
        }
        if (level <= 10) {
    ...
    public int getAccuracy(Player player) {
        ...
        return getAccuracyForLevel(player.getLevel());
    }
}
...
class TorchItem extends WeaponItem {
    public TorchItem() {
        super("Torch", 4, 80, "Torch",
                "Weapon and utility item. Strong against slime and useful for dark rooms.");
    }
...
class DecayedArmorItem extends PassiveGearItem {
    public DecayedArmorItem() {
        super("Decayed Armor", "Worn-out armor that still offers some protection. Passive +3 Defense while carried.", 0, 3, false);
    }
}

class ArmorItem extends PassiveGearItem {
    public ArmorItem() {
        super("Armor", "Covers most vital body parts, knight's best defense. Passive +6 Defense while carried.", 0, 6, false);
    }
}

class ShieldItem extends PassiveGearItem {
    public ShieldItem() {
        super("Shield", "Round, one hand small shield. Passive +3 Defense while carried. Allows to Parry some attacks.", 0, 3, true);
    }
...
class DragonScaleShieldItem extends PassiveGearItem {
    public DragonScaleShieldItem() {
        super("Dragon Scale", "A dragon scale from defeated dragon can be used as shield. Passive +8 Defense while carried. Allows to Parry some attacks.", 0, 8, true);
    }
